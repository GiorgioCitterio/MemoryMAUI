...
public partial class DifficileAndroid : ContentPage
{
    ...
    public DifficileAndroid(IAudioManager audioManager)
    {
        InitializeComponent();
        sw.Start();
        ...
        if (contCarteGir >= 2)
        {
            ...
            if (matricePosNumeri[Grid.GetRow(image), Grid.GetColumn(image)] == matricePosNumeri[Grid.GetRow(cartaGirata), Grid.GetColumn(cartaGirata)])
            {
                ...
                if (coppieTrovate == 18)
                {
                    ...
                    SceltaTemaAndroid.dato.Difficolta = Difficolta.Difficile;
